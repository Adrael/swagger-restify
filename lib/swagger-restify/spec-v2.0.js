...
SpecV20.prototype._createHandleSwaggerApiCb = function (_resourceRef, done) {
    return function (api) {
        ...
        } else if (api.definitions || api.responses) {
            if (api.definitions) _resourceRef.definitions = api.definitions;
            if (api.responses) _resourceRef.responses = api.responses;
        } else {
    ...
    if (apis) {
        ...
        async.eachSeries(apis, function (api, done) {
            self._readApi(api, function (err, resource) {
                ...
                _.extend(self.spec.responses, resource.responses);
